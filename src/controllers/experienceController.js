...
export const crud = createCRUD(Experience)

export const listPublic = async (req, res) => {
  try {
    const items = await Experience.find({ visible: true }).sort({ displayOrder: 1, createdAt: 1 })
...
export const reorder = async (req, res) => {
  try {
    ...
    const updated = await Experience.find().sort({ displayOrder: 1 })
